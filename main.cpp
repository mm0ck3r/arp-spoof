	...
	for(int i=0;i<cnt;i++){
		...
		while(1){ 
			...
			if(eth_type == EthHdr::Arp){
				...
				if (static_cast<uint32_t>(arp_hdr->sip()) == static_cast<uint32_t>(Ip(arg[i+2]))){
                    break;
                }
			}
		}
...
int main(int argc, char* argv[]) {
	...
	GetMacAddrImsi(argv, infect_cnt*2);

	for(int i=0;i<infect_cnt;i++){
		if(!send_packet_arp(sender_mac[i],my_mac,sender_mac[i],Ip(target_ip[i]),Ip(sender_ip[i]),false)) printf("Infect! (INIT)\n");
	}
